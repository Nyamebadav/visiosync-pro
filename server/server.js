...
app.get("/health", (_req, res) => {
  ...
  res.json({
    ...
    hasHFKey: hasHF,
    status: {
      claude: hasClaude ? "✓ Claude AI ready" : "✗ Add ANTHROPIC_API_KEY",
      chatgpt: hasOpenAI ? "✓ ChatGPT ready" : "✗ Add OPENAI_API_KEY",
      qwen: hasQwen ? "✓ Qwen ready" : "✗ Add QWEN_API_KEY",
      mimo: hasHF ? "✓ MiMo ready" : "✗ Add HF_API_KEY",
...
// ── OpenAI / ChatGPT text generation proxy ───────────────────────────────
...
// ── Qwen (Alibaba DashScope) text generation proxy ───────────────────────
...
// ── Xiaomi MiMo text generation proxy (HuggingFace Inference API) ─────────
...
app.listen(port, "0.0.0.0", () => {
  ...
  console.log(`  OPENAI_API_KEY:    ${process.env.OPENAI_API_KEY ? "✓ loaded" : "- not set (optional)"}`);
  console.log(`  QWEN_API_KEY:      ${process.env-QWEN_API_KEY ? "✓ loaded" : "- not set (optional)"}`);
  console.log(`  HF_API_KEY:        ${process.env.HF_API_KEY ? "✓ loaded" : "- not set (optional)"}\n`);
});
